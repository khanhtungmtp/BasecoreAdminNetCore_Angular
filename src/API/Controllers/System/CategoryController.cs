...
public class CategoryController(I_Category categoryService) : BaseController
{
    ...
    {
        OperationResult<string>? result = await _categoryService.CreateAsync(request);
        if (result.Succeeded)
        {
    ...
    {
        OperationResult<CategoryVM>? result = await _categoryService.FindByIdAsync(id);
    ...
    {
        OperationResult<PagingResult<CategoryVM>>? result = await _categoryService.GetPagingAsync(filter, pagination, categoryVM);
        return HandleResult(result);
    }
    ...
    {
        OperationResult? result = await _categoryService.PutAsync(id, request);
         return HandleResult(result);
    }
    ...
    {
        OperationResult<string>? result = await _categoryService.DeleteAsync(id);
         return HandleResult(result);
    }
}
